...
exports.author_list = function(req, res, next) {
  async.parallel({
    authors: function(callback) {
      Author.find({}).select(['firstname', 'lastname', 'username']).exec(callback);
    }
  ...
  jwt.verify(req.token, 'secret', (err, authData) => {
    ...
    function(err, results) {
      ...
      if (results.posts.length > 0) {
        ...
        res.json({
          message: "Please delete all of the posts by this author before deleting the author."
        })
        return;
      }
...
exports.author_update = [
  ...
  body('username', 'Username is already in use.').custom((value, { req }) => {
    return new Promise((resolve, reject) => {
      Author.findOne({ username: req.body.username }, function(err, author) {
        if (err) return next(err);
        if (author && author.username == value && author.username != req.authData.username) {
          reject(new Error('Username is already in use.'))
        }
        resolve(true);
      });
    ...
    function(err, results) {
      ...
      if (!errors.isEmpty()) {
        ...
        bcrypt.hash(req.body.password, 10, (err, hashedPassword) => {
          ...
          Author.findByIdAndUpdate(req.params.id, author, { new: true }, function(err, newAuthor) {
            if (err) return next(err);
            res.json({
